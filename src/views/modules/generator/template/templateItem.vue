<template>
  <table-view :show-btn="false">
    ...
    >
      ...
      <el-table-column prop="name" label="模板名" width="220" />
      <el-table-column prop="moduleName" label="模块名" width="280" />
    ...
    <dialog-view slot="other" ref="dialogRef" :title="dialogParams.title" :width="dialogParams.width" :fullscreen="true">
...
export default {
    ...
    data() {
        return {
            searchApi: this.$api.generator.template.list,
            deleteApi: this.$api.generator.template.delete,
            title: '模板',
            form: {
                code: '',
                name: ''
    ...
    methods: {
        handleCustomAdd(row) {
            if (row.constructor.name === 'MouseEvent') {
                this.doInitDialog({ templateGroupId: this.templateGroupId }, 'add', '模板', this.$store.getters.mobile ? '80%' : '70%')
            } else {
                this.$ajax.get(this.$api.generator.template.get, { id: row.id }).then(result => {
                    row.templateGroupId = this.templateGroupId
                    row.content = result.bean.content
                    this.doInitDialog(row, 'add', '模板', this.$store.getters.mobile ? '80%' : '70%')
                })
            }
        },
